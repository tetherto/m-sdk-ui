...
// ============================================================================
// Time Attribute Parsing
// ============================================================================

/**
 * Get the attribute name with the smallest time unit from a data object
 *
 * Parses keys like `t_30s`, `t_1m`, `t_5m`, `t_15m` and returns
 * the one representing the smallest time interval.
 *
 * @example
 * ```ts
 * getAttributeWithSmallestTime({ avg: 0, t_30s: 10, t_1m: 20, t_5m: 30 })
 * // 't_30s'
 * ```
 */
export const getAttributeWithSmallestTime = (data: Record<string, number>): string | undefined => {
  ...
  for (const key of Object.keys(data)) {
    ...
    if (timeframe.endsWith('m')) {
      seconds = Number.parseFloat(timeframe) * 60
    } else if (timeframe.endsWith('s')) {
      seconds = Number.parseFloat(timeframe)
    } else {
      continue
    }
